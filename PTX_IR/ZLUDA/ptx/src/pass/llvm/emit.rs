...
pub(crate) fn run<'input>(
    ...
    let mut emit_ctx = ModuleEmitContext::new(context, &module, &id_defs, fp_mode, sm_version);
...
impl<'a, 'input> ModuleEmitContext<'a, 'input> {
    ...
    fn emit_method(
        ...
        if fn_ == ptr::null_mut() {
            ...
            self.emit_fn_attribute_string(fn_, "no-trapping-math", "true");
            if self.fp_mode == FloatingPointMode::Constrained {
                self.emit_fn_attribute_enum(fn_, "strictfp");
            }
        }
        ...
        if let Some(kernel_attrs) = &method.kernel_attributes {
            ...
            self.emit_fn_attribute_string(fn_, "denormal-fp-math-f32", "dynamic");
            self.emit_fn_attribute_string(fn_, "denormal-fp-math", "dynamic");
        }
        ...
        for directive in method.tuning {
            match directive {
                ptx_parser::TuningDirective::NoReturn => {
                    self.emit_fn_attribute_string(fn_, "noreturn", "true")
                }
                _ => {
    ...
    fn emit_fn_attribute_string(&self, llvm_object: LLVMValueRef, key: &str, value: &str) {
    ...
    fn emit_fn_attribute_enum(&self, llvm_object: LLVMValueRef, key: &str) {
        let attr_kind = unsafe { LLVMGetEnumAttributeKindForName(key.as_ptr().cast(), key.len()) };
        let attribute = unsafe { LLVMCreateEnumAttribute(self.context, attr_kind, 0) };
        unsafe { LLVMAddAttributeAtIndex(llvm_object, LLVMAttributeFunctionIndex, attribute) };
    }
        ...
        for tuning in tuning_directives {
            match tuning {
                ...
                ptx_parser::TuningDirective::MaxNtid(x, y, z) => {
                    ...
                    // NVPTX expresses block-size constraints via metadata; skip for now
                    let _ = value;
                }
                ...
                ptx_parser::TuningDirective::MinNCtaPerSm(_ctas) => {
                    // no NVPTX IR attribute equivalent for min-ctas-per-sm
                }
            }
        }
    }
    ...
    fn emit_target_features(&mut self, fn_: LLVMValueRef) {
        // NVPTX uses target-cpu to select the SM version
        let cpu = format!("sm_{}", self.sm_version);
        self.emit_fn_attribute_string(fn_, "target-cpu", &cpu);
    }
...
impl<'a> MethodEmitContext<'a> {
    ...
    fn emit_kernel_prelude(
        ...
        if cfg!(windows) {
            // dcache invalidation has no NVPTX equivalent; skip
        }
    ...
    fn emit_add(
        ...
        match data {
            ast::ArithDetails::Integer(ast::ArithInteger {
                ...
                return self.emit_intrinsic_saturate(op, type_, arguments.dst, src1, src2);
            }
            ast::ArithDetails::Integer(ast::ArithInteger {
                saturate: false, ..
            }) => {
                self.resolver.with_result(arguments.dst, |dst| unsafe {
                    LLVMBuildAdd(builder, src1, src2, dst)
                });
            }
            ast::ArithDetails::Float(..) => {
                self.fp
                    .fadd(self.resolver, data.type_(), arguments.dst, src1, src2)?;
            }
        };
        Ok(())
    }
    ...
    fn emit_mul_impl(
        ...
        let mul_fn = match data {
            ...
            ast::MulDetails::Float(..) => {
                return self.fp.fmul(self.resolver, data.type_(), dst, src1, src2)
            }
        };
    ...
    fn emit_intrinsic_with_metadata(
        &mut self,
        name: &CStr,
        dst: Option<SpirvWord>,
        return_types: Vec<&ast::Type>,
        mut arguments: Vec<(LLVMValueRef, LLVMTypeRef)>,
        metadata: &[&str],
    ) -> Result<LLVMValueRef, TranslateError> {
        let has_metadata_args = !metadata.is_empty();
        let metadata_type = unsafe { LLVMMetadataTypeInContext(self.context) };
        let metadata = metadata.iter().map(|m| {
            let m_value =
                unsafe { LLVMMDStringInContext2(self.context, m.as_ptr().cast(), m.len()) };
            (
                unsafe { LLVMMetadataAsValue(self.context, m_value) },
                metadata_type,
            )
        });
        if has_metadata_args {
            arguments = arguments.into_iter().chain(metadata).collect::<Vec<_>>();
        }
    ...
    fn emit_rsqrt(
        ...
        let intrinsic = match (data.type_, self.fp.mode) {
            (ast::ScalarType::F32, FloatingPointMode::Normal) => c"llvm.nvvm.rsqrt.approx.f",
            (ast::ScalarType::F32, FloatingPointMode::Constrained) => {
                c"llvm.nvvm.rsqrt.approx.ftz.f"
            }
            (ast::ScalarType::F64, FloatingPointMode::Normal) => c"llvm.nvvm.rsqrt.approx.d",
            (ast::ScalarType::F64, FloatingPointMode::Constrained) => {
                c"llvm.nvvm.rsqrt.approx.d"
            }
            _ => return Err(error_unreachable()),
        };
        self.emit_intrinsic(
    ...
    fn emit_sqrt(
        ...
        let type_ = get_scalar_type(self.context, data.type_);
        let (intrinsic, metadata): (_, &[&str]) = match (data.type_, data.kind, self.fp.mode) {
            (ast::ScalarType::F32, ast::RcpKind::Approx, FloatingPointMode::Normal) => {
                (c"llvm.nvvm.sqrt.approx.f", &[])
            }
            (ast::ScalarType::F32, ast::RcpKind::Approx, FloatingPointMode::Constrained) => {
                (c"llvm.nvvm.sqrt.approx.ftz.f", &[])
            }
            (ast::ScalarType::F32, ast::RcpKind::Compliant(..), FloatingPointMode::Normal) => {
                (c"llvm.sqrt.f32", &[])
            }
            ...
            (ast::ScalarType::F64, ast::RcpKind::Compliant(..), FloatingPointMode::Normal) => {
                (c"llvm.sqrt.f64", &[])
            }
    ...
    fn emit_rcp(
        ...
        let intrinsic = match (data.type_, data.kind, self.fp.mode) {
            (ast::ScalarType::F32, ast::RcpKind::Approx, FloatingPointMode::Normal) => {
                c"llvm.nvvm.rcp.approx.f"
            }
            (ast::ScalarType::F32, ast::RcpKind::Approx, FloatingPointMode::Constrained) => {
                c"llvm.nvvm.rcp.approx.ftz.f"
            }
            (ast::ScalarType::F64, ast::RcpKind::Approx, FloatingPointMode::Normal) => {
                c"llvm.nvvm.rcp.approx.d"
            }
    ...
    fn emit_ex2(
        ...
        let intrinsic = match (data.type_, self.fp.mode) {
            (ast::ScalarType::F16, FloatingPointMode::Normal) => c"llvm.nvvm.ex2.approx.f16",
            (ast::ScalarType::F16, FloatingPointMode::Constrained) => {
                c"llvm.nvvm.ex2.approx.f16"
            }
            (ast::ScalarType::F32, FloatingPointMode::Normal) => c"llvm.nvvm.ex2.approx.f",
            (ast::ScalarType::F32, FloatingPointMode::Constrained) => {
                c"llvm.nvvm.ex2.approx.ftz.f"
            }
            _ => return Err(error_unreachable()),
        };
        self.emit_intrinsic(
    ...
    fn emit_lg2(
        ...
        let intrinsic = match self.fp.mode {
            FloatingPointMode::Normal => c"llvm.nvvm.lg2.approx.f",
            FloatingPointMode::Constrained => c"llvm.nvvm.lg2.approx.ftz.f",
        };
        self.emit_intrinsic(
            intrinsic,
            Some(arguments.dst),
    ...
    fn emit_bar_warp(&mut self) -> Result<(), TranslateError> {
        // llvm.nvvm.bar.warp.sync takes a membermask (all lanes = 0xffffffff)
        let llvm_i32 = unsafe { LLVMInt32TypeInContext(self.context) };
        let all_lanes = unsafe { LLVMConstInt(llvm_i32, 0xffffffff, 0) };
        self.emit_intrinsic(c"llvm.nvvm.bar.warp.sync", None, vec![], vec![(all_lanes, llvm_i32)])?;
        Ok(())
    }
    ...
    fn emit_mul24(
        ...
        let name_lo = match data.type_ {
            ast::ScalarType::U32 => c"llvm.nvvm.mul24.ui",
            ast::ScalarType::S32 => c"llvm.nvvm.mul24.i",
            _ => return Err(error_unreachable()),
        };
        ...
        if data.control == Mul24Control::Hi {
            // mul24.hi: bits 47..16 of the 48-bit product.
            // NVPTX has no mulhi.u24/i24 intrinsic, so widen to 64-bit, multiply, shift.
            let (ext_build, shr_build): (
                unsafe extern "C" fn(LLVMBuilderRef, LLVMValueRef, LLVMTypeRef, *const i8) -> LLVMValueRef,
                unsafe extern "C" fn(LLVMBuilderRef, LLVMValueRef, LLVMValueRef, *const i8) -> LLVMValueRef,
            ) = if data.type_ == ast::ScalarType::S32 {
                (LLVMBuildSExt, LLVMBuildAShr)
            } else {
                (LLVMBuildZExt, LLVMBuildLShr)
            };
            let i64_ty = unsafe { LLVMInt64TypeInContext(self.context) };
            let i32_ty = get_scalar_type(self.context, data.type_);
            let src1_w = unsafe { ext_build(self.builder, src1, i64_ty, LLVM_UNNAMED.as_ptr()) };
            let src2_w = unsafe { ext_build(self.builder, src2, i64_ty, LLVM_UNNAMED.as_ptr()) };
            let prod = unsafe { LLVMBuildMul(self.builder, src1_w, src2_w, LLVM_UNNAMED.as_ptr()) };
            let shift16 = unsafe { LLVMConstInt(i64_ty, 16, 0) };
            let shifted = unsafe { shr_build(self.builder, prod, shift16, LLVM_UNNAMED.as_ptr()) };
            let truncated = unsafe { LLVMBuildTrunc(self.builder, shifted, i32_ty, LLVM_UNNAMED.as_ptr()) };
            self.resolver.register(arguments.dst, truncated);
        }
        Ok(())
    }

    fn emit_set_mode(&mut self, _mode_reg: ModeRegister) -> Result<(), TranslateError> {
        // NVPTX encodes rounding/denormal mode per-instruction; no runtime setreg equivalent
        Ok(())
    }
    ...
    fn emit_dp4a(
        ...
        let intrinsic = match (data.atype, data.btype) {
            (ast::ScalarType::U32, ast::ScalarType::U32) => c"llvm.nvvm.idp4a.u.u",
            (ast::ScalarType::S32, ast::ScalarType::S32) => c"llvm.nvvm.idp4a.s.s",
        ...
        self.emit_intrinsic(
            ...
            vec![
                ...
                // llvm.nvvm.idp4a.*.* takes no clamp flag (unlike amdgcn.udot4/sdot4)
            ],
        )?;
        Ok(())
    }
...
impl FloatingPoint {
    fn emit_constrained<const N: usize, const M: usize>(
        &self,
    ...
    fn emit_constrained_impl<const N: usize, const M: usize>(
        ...
        emit_intrinsic(
            ...
            vec![&dst_type.into()],
            srcs.iter()
                .map(|&src| (src, get_scalar_type(self.context, src_type)))
    ...
    fn fadd(
        ...
        match self.mode {
            FloatingPointMode::Normal => Ok(resolver.with_result(dst, |dst| unsafe {
                LLVMBuildFAdd(self.builder, src1, src2, dst)
            })),
    ...
    fn fmul(
        ...
        let src1 = resolver.value(src1)?;
        let src2 = resolver.value(src2)?;
        match self.mode {
            FloatingPointMode::Normal => Ok(resolver.with_result_option(dst, |dst| unsafe {
                LLVMBuildFMul(self.builder, src1, src2, dst)
            })),
            FloatingPointMode::Constrained => self.emit_constrained(
                resolver,
    ...
    fn fdiv(
        ...
        match self.mode {
            FloatingPointMode::Normal => Ok(resolver.with_result(dst, |dst| unsafe {
                LLVMBuildFDiv(self.builder, src1, src2, dst)
            })),
            FloatingPointMode::Constrained => self.emit_constrained(
                resolver,
...
fn emit_intrinsic(
    ...
    let mut fn_ = unsafe { LLVMGetNamedFunction(module, name.as_ptr()) };
    if fn_ == ptr::null_mut() {
        fn_ = unsafe { LLVMAddFunction(module, name.as_ptr(), fn_type) };
    }
    let mut arguments = arguments.iter().map(|(arg, _)| *arg).collect::<Vec<_>>();
    Ok(resolver.with_result_option(dst, |dst| unsafe {
        LLVMBuildCall2(
            builder,
            fn_type,
            fn_,

#!/bin/bash
# =============================================================================
# PTX→SASS 1:1 映射验证 - 一键执行全流程
#
# 步骤:
#   1. 生成 PTX 测试用例 (generate_ptx.py)
#   2. 批量编译 (compile_all.sh)
#   3. 批量反汇编 (disasm_all.sh)
#   4. 自动化分析 (analyze.py)
#
# 用法: bash scripts/run_all.sh [--arch sm_100]
# =============================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE_DIR="$(dirname "$SCRIPT_DIR")"

ARCH="${1:-sm_100}"
if [[ "$1" == "--arch" ]]; then
    ARCH="${2:-sm_100}"
fi

echo "================================================================"
echo "  PTX→SASS 1:1 Mapping Verification Pipeline"
echo "  Architecture: $ARCH"
echo "================================================================"
echo ""

# Step 1: Generate PTX
echo "[Step 1/4] Generating PTX test cases..."
python3 "$SCRIPT_DIR/generate_ptx.py"
echo ""

# Step 2: Compile
echo "[Step 2/4] Compiling PTX -> cubin (O0 + O3)..."
bash "$SCRIPT_DIR/compile_all.sh" --arch "$ARCH" --continue-on-error --verbose
echo ""

# Step 3: Disassemble
echo "[Step 3/4] Disassembling cubin -> SASS..."
bash "$SCRIPT_DIR/disasm_all.sh"
echo ""

# Step 4: Analyze
echo "[Step 4/4] Analyzing SASS output..."
python3 "$SCRIPT_DIR/analyze.py" --output "$BASE_DIR/results/mapping_report.csv"
echo ""

echo "================================================================"
echo "  Pipeline complete!"
echo "  Report: $BASE_DIR/results/mapping_report.csv"
echo "================================================================"

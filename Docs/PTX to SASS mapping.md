...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZDcyNjBiMmE4OGYxNTQzODI2YWFmZjhjMWJlNTk5ZjFfZTI3MjQyMDMxNzA5Mjg0OWM1MjNkYzYzZjcyNjUzNWVfSUQ6NzY2NTUzMjUxNzM0ODc0MDA2OF8xNzg0ODYyMjk2OjE3ODQ5NDg2OTZfVjM)
...
- `LDC / ISETP.EQ / @!P0 BRA`：读取运行时 cluster 配置并选择 cluster 路径或兼容路径；

- `WARPSYNC.COLLECTIVE(.ALL) / ENDCOLLECTIVE`：在 O0 lowering 中包围 barrier 操作，保证 warp 内参与线程以 collective 方式执行；

- `MEMBAR.ALL.GPU + ERRBAR + CGAERRBAR`：位于 `UCGABAR_ARV` 之前，完成到达操作所需的内存顺序及 cluster 相关状态处理；

#!/usr/bin/env python3
"""Self-contained Thor experiment runtime template for one PTX opcode suite.

Copy this file verbatim into `<family>/<opcode>/thor_ptx90/` next to a
`suite_spec.py` that defines `SPEC`. All family/opcode naming derives from
`Spec.family` / `Spec.opcode` / `Spec.ptx_opcode`; the copy stays unmodified so
per-suite behavior lives entirely in `suite_spec.py`. Runtime semantics are
outside pass criteria. Negative probes may pin an `expected_diagnostic`
substring so a rejection is attributed to the constraint it claims to test.
"""
...
OWNER_MARKER = ".ptx-sass-suite-owner.json"
...
def reset_owned_directory(path: Path, owner: str, protected: Iterable[Path] = ()) -> Path:
    ...
    marker.write_text(
        json.dumps({"schema_version": "ptx_sass_suite_owner_v1", "owner": owner}) + "\n",
        encoding="utf-8",
    )
    return resolved
...
def module_source(case: Case, ptx_opcode: str) -> str:
    lines = [
...
def _case_id(spec: Spec, suffix: str, index: int, coords: dict) -> str:
    short = (spec.opcode + suffix).replace("_", "")
    return f"thor_{spec.family}_{short}_{index:04d}_{stable_hash(coords)[:8]}"


def materialize_cases(spec: Spec, mode: str) -> list[Case]:
    raw = spec.syntax_cases() if mode == "syntax" else spec.expanded_cases()
    return [Case(_case_id(spec, "", index, case.coordinates), case.coordinates, case.declarations, case.parameters, case.registers, case.preparation, case.target, case.observation, case.directives, case.expected, case.reason, case.expected_diagnostic) for index, case in enumerate(raw, 1)]


def generate(spec: Spec, mode: str, output: Path, suite_root: Path) -> dict:
    output = reset_owned_directory(output, f"thor_{spec.family}_{spec.opcode}_generated", (suite_root,))
    rows = []
    for case in materialize_cases(spec, mode):
        source = module_source(case, spec.ptx_opcode)
        path = output / f"{case.label}.ptx"
        path.write_text(source, encoding="utf-8")
        rows.append({"schema_version": f"{spec.family}_static_manifest_v1", "case_id": case.label, "case_label": case.label, "kernel": case.label, "opcode": spec.ptx_opcode, "mode": mode, "coordinates": case.coordinates, "semantic_form": case.coordinates, "semantic_form_id": stable_hash(case.coordinates), "source_variant": {"mode": mode}, "source_variant_id": stable_hash({"mode": mode}), "source_implementation_id": stable_hash({"case": case.label, "source": source}), "target_instructions": list(case.target), "target_occurrence_count": len(case.target), "validation_scope": "STATIC_ATTRIBUTION", "environment_id": "thor_cuda13_ptx90_sm110a", "source_file": path.name, "source_sha256": hashlib.sha256(source.encode()).hexdigest(), "expected": case.expected})
    (output / "manifest.jsonl").write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in rows), encoding="utf-8")
    summary = {"schema_version": f"{spec.family}_generation_summary_v1", "opcode": spec.ptx_opcode, "mode": mode, "case_count": len(rows), "manifest_sha256": stable_hash(rows)}
...
def validate_sources(spec: Spec, source_dir: Path) -> dict:
    ...
    for row in rows:
        ...
        if spec.ptx_opcode not in text: errors.append(f"opcode marker missing {path.name}")
...
def compile_suite(spec: Spec, suite_root: Path, mode: str, work_dir: Path, ptxas: Path, nvdisasm: Path, jobs: int, optimizations: tuple[str, ...], summary_output: Path | None) -> dict:
    work_dir = reset_owned_directory(work_dir, f"thor_{spec.family}_{spec.opcode}_check", (suite_root,))
    ...
    if not compile_failures:
        for result in results:
            ...
            attributions.append({"schema_version": f"{spec.family}_static_attribution_v1", "case_id": case_id, "case_label": case_id, "occurrence_index": 0, "ptx_instruction": "\n".join(manifest[case_id]["target_instructions"]), "status": "ATTRIBUTED" if targets else "TARGET_ELIMINATED", "observation_level": "exact", "sass_instructions": [item["operation"] for item in targets], "encoding_words": [word for item in targets for word in item["encoding_words"]], "environment_id": "thor_cuda13_ptx90_sm110a", "opcode": spec.ptx_opcode, "optimization": result["optimization"], "coordinates": manifest[case_id]["coordinates"], "instruction_count": len(instructions), "target_patterns": list(spec.target_patterns), "target_instructions": targets, "full_instructions": instructions, "sass_file": sass_path.name})
...
def check_negative(spec: Spec, suite_root: Path, work_dir: Path, ptxas: Path, summary_output: Path | None) -> dict:
    work_dir = reset_owned_directory(work_dir, f"thor_{spec.family}_{spec.opcode}_negative", (suite_root,))
    results = []
    for index, raw in enumerate(spec.negative_cases(), 1):
        case = Case(_case_id(spec, "_negative", index, raw.coordinates), raw.coordinates, raw.declarations, raw.parameters, raw.registers, raw.preparation, raw.target, raw.observation, raw.directives, raw.expected, raw.reason, raw.expected_diagnostic)
        source = work_dir / f"{case.label}.ptx"; cubin = work_dir / f"{case.label}.cubin"
        source.write_text(module_source(case, spec.ptx_opcode), encoding="utf-8")
        result = _compile_one(ptxas, source, cubin, "O0")
        rejected = result["returncode"] != 0
        diagnostic_matched = (case.expected_diagnostic in result["stderr"]) if case.expected_diagnostic else None
        passed = rejected and diagnostic_matched is not False
        results.append({"case_id": case.label, "coordinates": case.coordinates, "reason": case.reason, "expected": "reject", "expected_diagnostic": case.expected_diagnostic, "diagnostic_matched": diagnostic_matched, "observed_returncode": result["returncode"], "stderr": result["stderr"], "status": "PASS" if passed else "FAIL"})
    failures = [row for row in results if row["status"] != "PASS"]
    report = {"schema_version": f"{spec.family}_negative_probe_report_v1", "validation_status": "PASS" if not failures else "FAIL", "opcode": spec.ptx_opcode, "probe_count": len(results), "expected_rejection_pass_count": len(results) - len(failures), "results": results}
...
def analyze(spec: Spec, manifest_path: Path, attribution_path: Path, output_dir: Path, suite_root: Path) -> dict:
    output_dir = reset_owned_directory(output_dir, f"thor_{spec.family}_{spec.opcode}_analysis", (suite_root,))
...
def write_factors(spec: Spec, suite_root: Path) -> None:
    ...
    for factor in spec.factors:
        ...
        item.setdefault("description", f"Controlled factor {item['id']} for {spec.ptx_opcode}.")
        item.setdefault("applicability", "See generated case coordinates and PTX grammar constraints.")
        factors.append(item)
    data = {"schema_version": "ptx_sass_factors_v1", "family_id": f"{spec.family}.{spec.ptx_opcode}.thor_ptx90", "opcode": spec.ptx_opcode, "environment_id": "thor_cuda13_ptx90_sm110a", "factors": factors, "constraints": [], "coverage": {"strategy": "mixed", "strength": 2, "optimizations": list(OPTIMIZATIONS), "high_risk_interactions": [], "stopping_condition": "all generated legal cases compile and have target attribution at O0-O3; all registered negative probes reject with the pinned diagnostic"}, "semantic_scope": "STATIC_ONLY"}
...
def cli_analyze(suite_root: Path) -> None:
    ...
    if args.summary_output:
        ...
        summary = {"schema_version": f"{spec.family}_static_mapping_summary_v1", "validation_status": "PASS", "opcode": result["opcode"], "environment": result["environment"], "forward_rule_count": result["forward_rule_count"], "inverse_signature_count": result["inverse_signature_count"], "rules_sha256": stable_hash(result["rules"]), "scope": result["scope"]}

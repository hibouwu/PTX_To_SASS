#!/usr/bin/env python3
"""Self-contained Thor experiment runtime for one TMA-family PTX opcode.

This local copy serves only this instruction directory; it deliberately has no
dependency on another instruction suite. Runtime semantics are outside pass
criteria. Structure mirrors the tcgen05 per-opcode suites with two additions:
`Spec.ptx_opcode` separates the full PTX opcode text from the short id used in
filenames, and negative probes may pin an `expected_diagnostic` substring so a
rejection is attributed to the constraint it claims to test.
"""
...
OWNER_MARKER = ".tma-suite-owner.json"
...
def reset_owned_directory(path: Path, owner: str, protected: Iterable[Path] = ()) -> Path:
    ...
    marker.write_text(
        json.dumps({"schema_version": "tma_suite_owner_v1", "owner": owner}) + "\n",
        encoding="utf-8",
    )
    return resolved
...
def compile_suite(spec: Spec, suite_root: Path, mode: str, work_dir: Path, ptxas: Path, nvdisasm: Path, jobs: int, optimizations: tuple[str, ...], summary_output: Path | None) -> dict:
    work_dir = reset_owned_directory(work_dir, f"thor_tma_{spec.opcode}_check", (suite_root,))
...
def check_negative(spec: Spec, suite_root: Path, work_dir: Path, ptxas: Path, summary_output: Path | None) -> dict:
    work_dir = reset_owned_directory(work_dir, f"thor_tma_{spec.opcode}_negative", (suite_root,))
    results = []
    for index, raw in enumerate(spec.negative_cases(), 1):
        case = Case(_case_id(spec.opcode + "_negative", index, raw.coordinates), raw.coordinates, raw.declarations, raw.parameters, raw.registers, raw.preparation, raw.target, raw.observation, raw.directives, raw.expected, raw.reason, raw.expected_diagnostic)
    ...
    report = {"schema_version": "tma_negative_probe_report_v1", "validation_status": "PASS" if not failures else "FAIL", "opcode": spec.ptx_opcode, "probe_count": len(results), "expected_rejection_pass_count": len(results) - len(failures), "results": results}
...
def analyze(spec: Spec, manifest_path: Path, attribution_path: Path, output_dir: Path, suite_root: Path) -> dict:
    output_dir = reset_owned_directory(output_dir, f"thor_tma_{spec.opcode}_analysis", (suite_root,))
...
def cli_analyze(suite_root: Path) -> None:
    ...
    if args.summary_output:
        ...
        summary = {"schema_version": "tma_static_mapping_summary_v1", "validation_status": "PASS", "opcode": result["opcode"], "environment": result["environment"], "forward_rule_count": result["forward_rule_count"], "inverse_signature_count": result["inverse_signature_count"], "rules_sha256": stable_hash(result["rules"]), "scope": result["scope"]}

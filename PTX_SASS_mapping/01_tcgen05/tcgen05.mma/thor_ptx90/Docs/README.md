...
- [`mapping_rules/README.md`](mapping_rules/README.md)：按 kind、CTA group、variant、操作数来源、collector、block scaling、`.ashift` 和内存一致性拆分的可检索规则索引；查单个语义维度时从这里开始；
...
- [`tcgen05_mma_PTX到SASS映射规则报告.md`](tcgen05_mma_PTX到SASS映射规则报告.md)：指令族综合报告，讲清 modifier 联合作用、完整 lowering、函数级 PTX/SASS 例子、寄存器和上下文；

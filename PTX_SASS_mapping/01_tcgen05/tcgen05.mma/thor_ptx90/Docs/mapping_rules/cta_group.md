# CTA group：`.cta_group::1/2` 如何进入 SASS

## 这个维度回答什么

CTA group 指一次 `tcgen05.mma` 由一个还是两个协作线程块（Cooperative Thread Array，CTA）参与。CTA 即 CUDA 的线程块（thread block）。

## 映射规则

| PTX | SASS |
|---|---|
| `.cta_group::1` | 主助记符不增加 CTA 数量修饰符 |
...
普通 MMA 的输出禁用 lane 掩码（disable-output-lane mask）数量随 CTA group 改变：
...
输出禁用 lane 掩码是指定哪些输出 lane 不写结果的位掩码（bitmask）。mask 数量变化属于 PTX 操作数形状的一部分，不能只靠 `.2CTA` 文本替代。

## 适用限制

- 普通 `mma` 和 `mma.sp` 覆盖 CTA group 1/2。
- 带分块缩放的普通 MMA 覆盖 CTA group 1/2。
...
`UTC*MMA` 代表 `UTCHMMA`、`UTCQMMA`、`UTCIMMA`、`UTCOMMA` 中由 kind 选中的家族。

group 2 还把输出禁用 lane 掩码从 4 个扩展到 8 个。根据 mask 如何产生，外围 SASS 从以下集合中选择：

| PTX/上下文作用 | 可能选择的 SASS | 作用 |
|---|---|---|
| 常量或普通寄存器 mask | `MOV`、`UMOV` | 形成每个 mask word |
| GPR mask 送入统一操作数 | `R2UR` | GPR 到统一寄存器搬运 |
...
基线说明 `.2CTA` 在 O0 和 O3 都稳定存在；零 mask 到 O3 已完全折叠。
...
这组展示 mask 的选择过程：O0 是 `MOV→R2UR`，O3 合并为 4 条或 8 条 `UMOV`。
...
O0：能看到 `IADD3`/`LOP3.LUT`/`R2UR` 生产链。O3 把恒等运算全部消去，最终只保留 `.2CTA` 差异：
...
`UTCBAR*` 只说明 tcgen05 异步工作被提交给完成协议；它与内存屏障（mbarrier）、release/acquire、显式 tcgen05 fence 和 LD/ST wait 的完整关系见 [`memory_consistency.md`](memory_consistency.md)。

### 对比 5：稀疏 INT8——跨变体、操作码与活跃寄存器
...
额外 4 个 mask 可使 O0 核心位置的活跃 UGPR 从 11 增至 15，但 O3 中准备序列被优化后两组的活跃寄存器数量重新相同。稳定保留的差异仍是 `.2CTA`。`.sp` 自身没有同名 `.SP` 后缀，稀疏语义由元数据（metadata）操作数和编码承载。

### 对比 6：分块缩放 4X——无 mask 的正交修饰符组合
...
把 group 2 与同一合法设计的 group 1 配对后，得到 2,432 个源码/上下文配对，即 O0–O3 共 9,728 次比较：
...
- 变化不是固定增加一条指令。观察到的 group 2 指令数增量为 8、16、24、32 或 40 条，取决于 mask、producer 和 completion 上下文。
- 没有观察到 group 2 比配对的 group 1 指令更少。

## 证据

- 52,736 条目标出现位置中，`.2CTA` 是否出现与 CTA group 完全一致。
- 当前样本反例为 0。
